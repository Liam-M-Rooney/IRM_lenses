...
This file will give an output of two figures:
    
Figure 1: the IRM image which can be used to compare to the 3d reconstruction

Figure 2:  is the 3d reconstruction of the lens with the theoretical 
    lens also implemented
    
the file will also print the max, the min, and the mean of the 
lens curvature
"""
...
'''
input IRM image file name into the variable img
'''
...
'''
N is the number of fringes read by the code, it takes some 
trial and error to find the max number of fringes detected by the code.
if you have too many fringes you will get an error message similar to:
...
# taken over on the IRM image 
        ...
        '''
        input scaling value to convert from pixels to distance (um/px)
        '''
        r_vals = r_vals * (847.87e-6 / 2048) 


        '''
        the variables for the find_ peak function can be copyed from calibration code
        '''
...
'''
the line below saves the lenses radius of curvature data,
bigR as a seperate txt file
...
'''
R is the theoretical radius of curvature in meters
z1 is the height of the theoretical lens
'''
R = 18.09e-3
z1 = 3.3e-3
...
while start < np.pi * 2:
    ...
    y1 = y1[:len(z1)]
    ax.plot3D(x1, y1, z1, color=((0,1,0)))
    i+=1
    start += np.pi /180
    end += np.pi /180


# this code checks that the theoretical lens curvature 
# is what is expected 
r1 = r_vals[:len(z1)]
z1 = z1[1:]
r1 = r1[1:]
