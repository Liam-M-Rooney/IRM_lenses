...
This code creates 3 images which can be used to get the correct settings 
when makeing the 3d image.

1. is the image of the lens with a line that the intensity is read from

2. is the intensity along the red line in figure 1, the peaks of this graph
    should be marked with an x to mark them

3. is a profile of the lens from the centre to the end of red line in figure 
...
'''
imput the name of the IRM image you want to look at below 
''' 
...
'''
when the code is first run you can look at the image to find the coordinates of
 the centre of the lens using ImageJ. 
If using spyder to look at the code the figures can be brought up in a seperate
window by using, 
    %matplotlib qt     and    %matplotlib inline (to reverse)
in the IPython console window. There will be the coordinates of you mouse 
in the bottem of the screen which can help to find the centre.
'''
# centre of lens
...
'''
angle is adjusted to check the intencity at different places on the lens 
(in radians) 
'''
...
'''
the lines below can be used to adjust which peaks are selected, i've used 
three variables in the function
